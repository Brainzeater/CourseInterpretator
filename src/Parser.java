...
public class Parser {
    ...
    public boolean expr() throws Exception {
        ...
        * присваивания переменной значения */
        if (declare() || assign() || whileOperation()
                || structDeclare() || forOperation()) {
            if (!postfixTokenList.isEmpty()) {
                setPoliz();
                poliz.go();
                postfixTokenList.clear();
...
//             Иначе отобразить сообщение об ошибке
    ...
    private boolean forOperation() throws Exception {
        if (forKw()) {
            ignoreWhitespace();
            if (bracketOpen()) {
                ignoreWhitespace();
                if (declare()) {
                    ...
                    if (condition()) {
                        ...
                        if (assign()) {
                            ...
                            ArrayList<Token> currentWhileIterationTokens = new ArrayList<Token>();
                            for(int i = 0; i < postfixTokenList.size(); i++){
                                currentWhileIterationTokens.add(postfixTokenList.get(i));
                            }
                            currentWhile.setIterationTokens(currentWhileIterationTokens);
                            postfixTokenList.clear();
                            ignoreWhitespace();
                            if (bracketClose()) {
                                if (poliz.conditionIsTrue()) {
                                    if (curlyBracketOpen()) {
                                        ...
                                        while (poliz.conditionIsTrue()) {
                                            ...
                                            if (tokens.get(currentTokenNumber).getName().equals("C_BRACKET_CLOSE")) {
                                                poliz.setPostfixToken(currentWhile.getIterationTokens());
                                                poliz.go();
                                                //
                                                currentTokenNumber = currentWhile.getMyStartingPoint();
                                                poliz.setPostfixToken(currentWhile.getConditionTokens());
                                                ...
                                                throw new Exception("for: missing right curly bracket");
                                            }
                                        }
                                        int curlyBracketsCounter = 0;
                                        currentToken = tokens.get(currentTokenNumber);
                                        ...
                                        throw new Exception("for: missing left curly bracket");
                                    }
                                } else {
                            /* Пропуск while, условие которого не выполняется */
                                    int curlyBracketsCounter = 0;
                                ...
                                throw new Exception("for: right bracket missing");
                            }
                        }

                    } else {
                        throw new Exception("for: condition missing");
                    }

                } else {
                    throw new Exception("for: declare missing");
                }

            } else {
                throw new Exception("for: left bracket missing");
            }
        } else {
            currentTokenNumber--;
            return false;
